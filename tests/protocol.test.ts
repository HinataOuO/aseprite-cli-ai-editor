...
const snapshot = {
  ...
  palette: [{ index: 0, rgba: 0 }], transparentIndex: 0, documentEmpty:false, selection: mask
};
...
test("protocol round-trip", () => {
  ...
  assert.deepEqual(validateSnapshot({...snapshot,width:72,height:48}).width,72);
...
test("rejects malformed exact masks and snapshots", () => {
  ...
  assert.equal(validateSnapshot({ ...snapshot, width: 17, height: 23 }).height,23);
  assert.throws(() => validateSnapshot({ ...snapshot, width: 129 }), /unsupported dimensions/);
  assert.throws(() => validateCandidate({ snapshotToken: "abc", bounds: mask.bounds, paletteRefs: [0] }));
  assert.throws(()=>validateSnapshot({...snapshot,usedRgba:[0xff0000ff,0xff0000ff]}),/unique/);
  assert.deepEqual(validateSnapshot({...snapshot,usedRgba:[0xff0000ff]}).usedRgba,[0xff0000ff]);
});
